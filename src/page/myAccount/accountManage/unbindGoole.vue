<template>
  <div class="warp-googleAtc">
    <div class="title">
      ...
      <div class="goto-google-btn" @click="gotoGoogle()">{{$t('bossGoogle.text13')}}</div>
    </div>
    <div class="main">
      <div class="goto-googleImg"><img src="../../../assets/images/login/isLock.png" alt=""></div>
      <div class="tit">{{googleHintText}}</div>
    ...
    <div class="mask-box" v-show="maskShow">
      <transition name="drop">
       <div class="content" v-if="maskShow">
        ...
        <form onsubmit="return false" class="formTable" action="">
          ...
          <div class="input-tit">{{$t('bossGoogle.text1')}}</div>
          <div class="input-box">
            <input type="number" @focus="payFailTip = ''" autocomplete="off" :placeholder="$t('bossGoogle.text10')" v-model.trim="emailCode">
        ...
        <div class="btn-box">
          ...
          <div class="confirm" @click="getUnlinkAjaxgoole()">{{$t('signIn.text10')}}</div>
        </div>
      </div>
      </transition>
    </div>
  ...
  export default {
    data() {
      return {
        ...
        codeHintShow: false,
        maskShow: false,
        googleHintText: this.$t('bossGoogle.text14')
      }
    },
    components: {
      smscode
    ...
    methods: {
      ...
      getUnlinkAjaxgoole () {
        if (this.payPassword === '') {
          this.pwdHintText = this.$t('sharer.text26')
        }
        if (this.emailCode === '') {
          this.payFailTip = this.$t('signIn.text20')
        }
        if (this.payPassword !== '' && this.emailCode !== '') {
          this.$postAxios.post(`${this.$api}tx/unlock`, { // 1.74 解锁帐户
            tradePwd: this.payPassword,
            mobileCode: this.emailCode,
            codeType: '1'
          }).then(res => {
            if (res.data.success === true) {
              this.$postAxios.post(`${this.$api}security/google_open_close`, { // 7 开启关闭Google验证码
                type: '2' // type 1:开启 2:关闭
              }).then(res => {
                if (res.data.success === true) {
                  this.maskShow = false
                  this.$store.dispatch('getUserBalance')
                  Dialog({
                    content: this.$t('bossGoogle.text16'),
                    okFn: () => {
                      this.$router.push('/account/googleAuthenticator')
                    }
                  })
                } else {
  ...
  .warp-googleAtc{
    ...
    .main{
      ...
      .tit{
        padding: 40px 0 30px;
        font-size: 24px;
        color: rgba(99, 224, 130, 1);
        text-shadow: 0 2px 2px rgba(99, 224, 130, .1);
        text-align: center;
      }
      .con{
        font-size: 16px;
