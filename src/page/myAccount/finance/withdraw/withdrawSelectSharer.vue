<template>
  <div class="selectSharer">
    <div class="bread">
      ...
      <router-link tag="i" to="/account/recharge">提现</router-link> 》
      <span>选择共享者</span>
    </div>
...
//        }
      ...
      // 获取共享者列表
      matchListByDeposit () {
        let url = `${this.$api2}pigame/matchListByDraw?id=${this.orderId}&sortBy=1&pageIndex=${this.pageIndex}&pageSize=${this.pageSize}`
      ...
      // 确定提现
      sure () {
        let url = `${this.$api2}draw/chooseBoss?id=${this.orderId}&bossId=${this.bossId}`
        this.$http.post(url).then(res => {
          if (res.data.status === 'success') {
            this.$router.push({
