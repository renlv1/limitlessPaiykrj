<template>
  <div class="recharge" >
    <div class="bread">
      <router-link tag="i" to="cashValue">{{$t('chongzhiTixian.text44')}}</router-link> 》 <span>{{$t('chongzhiTixian.text45')}}</span>
    </div>
    <h3>{{$t('chongzhiTixian.text45')}}</h3>
    <div class="recharge-box">
      <div class="rechargeNum">
        <span>{{$t('finance.text1')}}</span>
        <div class="inputBox">
          <input type="text" :placeholder="$t('finance.text2')" v-model="rechargeNum" @input="changeBuyNum('rechargeNum')" @blur="validata()">
          <div class="input-curreny">{{iptCurreny}}</div>
          <span v-if="rechargeNumerror">{{rechargeNumerror}}</span>
        </div>
      </div>
      <div class="currency">
        <span>{{$t('finance.text3')}}</span>
      ...
      <div class="btn" :class="{'active': validataFlag}" @click="createDepositOrder()">{{$t('finance.text6')}}</div>
    </div>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      selectedFlag: false,
      selectedCurrency: {
        text: this.$t('finance.text7'),
        tag: ''
      },
      payNumber: this.$t('finance.text4'),
      payPwd: '',
  ...
  methods: {
    ...
    changeCurrency (item) {
      ...
      this.getAccountList()
      // 充值、提现：
      // 支付/收款币种选择USD、CNY、HKD、CAD时，充值/提现数额的单位是USD
      // 支付/收款币种其他币种（BTC、LTC……）时，充值/提现数额的单位和支付/收款币种一样
    ...
    // 选择银行卡号
    selectBankCard (item) {
      this.bankType = item.accountType
    ...
    createDepositOrder () {
      ...
      let url = `${this.$api2}deposit/createOrder?currency=${this.selectedCurrency.tag}&amount=${this.rechargeNum}&type=${this.bankType}&remark=&bankAccountId=${this.selectedNumber.id}`
