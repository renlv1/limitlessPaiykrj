<template>
  <div class="myaccount-bill">
    <!--共享者订单-->
    <h3 class="my-title">{{$t('shareOrder.text1')}}</h3>
    <div class="flex-form">
    ...
    <div class="table-wrapper">
      <ul class="table-uls">
        ...
        <li class="table-list">{{$t('bill.text26')}}</li>
      ...
      <ul class="table-uls cursor" v-for="(item, index) in orderData" :key="index" @click="gotoInfo(item.id, item.orderState)">
        ...
        <li class="table-list">{{orderTypes(item, chongzhiStatus)}}</li>
...
export default {
  ...
  methods: {
    orderTypes (item, chongzhi) {
      if (chongzhi === 0 && item.payState === 1 && item.orderState === 2) {
    ...
    gotoInfo (id) {
      if (this.chongzhiStatus === 1) { // 提现详情
        ...
        this.$router.push({path: '/account/sharerOrder/shareRechargeOrderDetail', query: {id: id}})
      }
    },
    // 充值or提现
    isChongzhi (status) {
